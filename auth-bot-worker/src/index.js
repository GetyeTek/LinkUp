...
export class AuthSessionManager extends DurableObject {
  ...
  async fetch(request) {
    ...
    if (url.pathname === '/schedule-expiry') {
      const { chatId, messageId, delayMs } = await request.json();
      
      // Store reference coordinates in DO persistence
      await this.ctx.storage.put('session_data', { chatId, messageId });
  ...
  async alarm() {
    ...
    try {
      ...
      console.log(`[SessionManager] Expired magic link message ID: ${messageId}`);
    } catch (e) {
      console.error("[SessionManager] Failed to recess expired message:", e.message);
    }
  }
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // 1. Handle text commands and SIM transmissions
      if (update.message) {
